...
```
GET /tables/check?table_name=<表名>&required_columns=实例ID,实例类型
```

**参数：**
- `table_name`: 表名（必填）
- `required_columns`: 必需字段列表，逗号分隔（必填）

**CURL 示例：**
```bash
curl "http://localhost:5000/tables/check?table_name=水利对象及关系图谱_实体&required_columns=实例ID,实例类型"
```
...
```
GET /tables/entity-types?table_name=<表名>&type_column=实例类型
```

**参数：**
- `table_name`: 表名（必填）
- `type_column`: 类型字段名（必填）

**CURL 示例：**
```bash
curl "http://localhost:5000/tables/entity-types?table_name=水利对象及关系图谱_实体&type_column=实例类型"
```

**响应示例：**
```json
{
  "entity_types": ["河流", "湖泊", "水库"]
}
```
...
- `check_metadata(table_name, required_columns)` - 健康检查，验证表和字段
- `get_distinct_entity_types(table_name, type_column_name)` - 获取所有实体类型
